use near_sdk::{borsh::{self, BorshDeserialize, BorshSerialize}, json_types::{U128, U64}, PanicOnDefault, Promise, PromiseResult, require};
...
impl AtomicSwap {
	...
	pub fn new(amount: U128, recipient: AccountId, secret_hash: Vec<u8>, lock_time: u64) -> Self {
		require!(env::attached_deposit() >= amount.0, "You must deposit at least the amount");
	...
	fn commit(&mut self) -> Promise {
		Promise::new(self.recipient.clone())
			...
			.then(Promise::new(env::predecessor_account_id()).transfer(env::account_balance() - self.amount.0))
	}
	...
	#[private]
	fn resolve_transfer(&mut self) -> Promise {
		...
		if !must_revert {
			self.progress(State::Commit)
				.then(Promise::new(env::predecessor_account_id()).transfer(env::account_balance()))
		} else {
			panic_str("Can't commit, try again");
		}
	}
}
...
mod tests {
	...
	#[test]
	#[should_panic(expected = "Only the recipient can claim the funds unless the lock has surpassed")]
